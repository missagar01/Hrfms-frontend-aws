...
const fetchJoiningData = async () => {
  ...
  try {
    ...
    // Get indices for columns AQ (index 42)
    const columnAQIndex = 42; // Column AQ index
    ...
    const processedData = dataRows
      .map(row => ({
        ...
        // Add the specific column we need to filter by
        columnAQ: row[columnAQIndex]  // Column AQ value
      }))
      // Filter rows where AQ is not null
      .filter(item => item.columnAQ !== null && item.columnAQ !== undefined && item.columnAQ !== '');

    console.log('Processed JOINING data with AQ filter:', processedData);
    
    // Remove the status filter - only use the AQ column filter
