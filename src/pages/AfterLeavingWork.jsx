...
const AfterLeavingWork = () => {
  ...
  const fetchLeavingData = async () => {
    ...
    try {
      ...
      const processedData = dataRows.map(row => ({
        ...
        designation: row[10] || '',
        salary: row[11] || '',
        plannedDate: row[12] || '', // Column M
        actual: row[13] || '' // Column N
      }));

      // Pending: Column M NOT NULL and Column N NULL
      const pendingTasks = processedData.filter(
        task => task.plannedDate && !task.actual
      );
      setPendingData(pendingTasks);

      // History: Column M NOT NULL and Column N NOT NULL
  ...
  const renderTable = () => {
    const dataToShow = activeTab === 'pending' ? filteredPendingData : filteredHistoryData;
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-white">
          ...
          <tbody className="divide-y divide-white">
            {isLoading ? (
              <tr>
                <td colSpan={activeTab === 'pending' ? 7 : 8} className="px-6 py-12 text-center">
                  <div className="flex justify-center flex-col items-center">
                    <div className="w-6 h-6 border-4 border-indigo-500 border-dashed rounded-full animate-spin mb-2"></div>
                    <span className="text-gray-600 text-sm">
                      Loading {activeTab === 'pending' ? 'pending' : 'history'} data...
                    </span>
                  </div>
                </td>
              </tr>
              ...
              dataToShow.map((item, index) => (
                <tr key={index} className="hover:bg-white">
                  ...
                  {activeTab === 'history' && (
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.actual ? new Date(item.actual).toLocaleDateString() : '-'}
                    </td>
                  )}
                </tr>
              ))
              ...
              <tr>
                <td colSpan={activeTab === 'pending' ? 7 : 8} className="px-6 py-12 text-center">
                  <p className="text-gray-500">
                    No {activeTab === 'pending' ? 'pending' : 'history'} after resignation work found.
                  </p>
                </td>
              </tr>
            )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">After Resignation Work</h1>
      </div>
