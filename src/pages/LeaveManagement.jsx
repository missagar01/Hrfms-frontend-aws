...
const fetchEmployees = async () => {
  ...
  // Return in Google Sheets date format (MM/DD/YYYY)
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // Get current date in proper date format for Google Sheets
    const now = new Date();
    const formattedTimestamp = `${now.getMonth() + 1}/${now.getDate()}/${now.getFullYear()}`;

    const rowData = [
      formattedTimestamp,           // Timestamp (Column A) - Date format
      "",                          // Serial number (empty for auto-increment)
      formData.employeeId,         // Employee ID
      formData.employeeName,       // Employee Name
      formatDOB(formData.fromDate), // Leave Date Start (Column E) - Date format
      formatDOB(formData.toDate),   // Leave Date End (Column F) - Date format
...
const formatToDDMMYYYY = (dateStr) => {
  ...
  return `${month}/${day}/${year}`; // Changed to MM/DD/YYYY format for Google Sheets
};

      
if (editableDates.from && editableDates.from !== selectedRow.startDate) {
  currentRow[startDateIndex] = formatToDDMMYYYY(editableDates.from);
}

if (editableDates.to && editableDates.to !== selectedRow.endDate) {
  currentRow[endDateIndex] = formatToDDMMYYYY(editableDates.to);
}
  ...
  const fetchHodNames = async () => {
    try {
      const response = await fetch(
        'https://script.google.com/macros/s/AKfycbyWlc2CfrDgr1JGsJHl1N4nRf-GAR-m6yqPPuP8Oggcafv3jo4thFrhfAX2vnfSzLQLlg/exec?sheet=MASTER&action=fetch'
      );
      ...
      // Get HOD names from Column C (index 2), skip header row (index 0)
      const hodData = rawData.slice(1).map(row => row[2]?.toString().trim()).filter(name => name);
  ...
  useEffect(() => {
    fetchLeaveData();
    fetchEmployees();
    fetchHodNames(); // Add this line to fetch HOD names
  }, []);
