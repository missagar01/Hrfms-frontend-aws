...
const handleSubmit = async (e) => {
  ...
  try {
    // For Joining status, just show the modal without submitting
...
useEffect(() => {
  if (selectedItem && selectedItem.candidateDOB) {
    setJoiningFormData(prev => ({
      ...prev,
      dobAsPerAadhar: selectedItem.candidateDOB
    }));
  }
}, [selectedItem]);
...
const handleJoiningSubmit = async (e) => {
  ...
  try {
    ...
    const followUpRowData = [
      formattedTimestamp,
      selectedItem.candidateEnquiryNo || '',
      'Joining', // Status is always Joining in this case
      formData.candidateSays,
      formatDOB(formData.nextDate) || '',
    ];

    // Format the planned date in dd/mm/yyyy hh:mm:ss format
    ...
    const seconds = String(plannedDate.getSeconds()).padStart(2, '0');
    const formattedPlannedDate = now.toLocaleString("en-US", {
      year: "numeric",
      month: "numeric",
      day: "numeric",
      hour: "2-digit",
    ...
    for (const field of fileFields) {
      if (joiningFormData[field] && typeof joiningFormData[field] === 'object') {
        uploadPromises[field] = uploadFileToDrive(joiningFormData[field]);
      } else if (joiningFormData[field]) {
        uploadPromises[field] = Promise.resolve(joiningFormData[field]);
      } else {
    ...
    const joiningTimestamp = now.toLocaleString("en-US", {
      year: "numeric",
      month: "numeric",
      day: "numeric",
      hour: "2-digit",
    ...
    rowData[0] = joiningTimestamp;           // Timestamp
    ...
    rowData[12] = selectedItem.candidatePhoto || ''; // Candidate Photo from ENQUIRY Sheet (Column M)
    ...
    rowData[39] = selectedItem.candidateResume || fileUrls.resumeCopy; // Resume from ENQUIRY Sheet or uploaded (Column AN)
    
    // Add empty values for columns AO, AP if needed
    rowData[40] = ''; // Column AO
    rowData[41] = ''; // Column AP
    
    // Add planned date to Column AQ (index 42)
    // rowData[42] = formattedPlannedDate;
