...
const fetchJoiningData = async () => {
  ...
  try {
    ...
    const processedData = dataRows.map(row => ({
      ...
      leavingDate: row[getIndex('Leaving Date')] || '', // Added Leaving Date column
    }));

    // Filter for records where Planned Date is not null and Leaving Date is null
    const completedTasks = processedData.filter(
      (task) => task.plannedDate && !task.leavingDate
    );
...
const handleSubmit = async(e) => {
  ...
  try {
    ...
    // First: Insert into LEAVING sheet
    const rowData = [
      formattedTimestamp,
      ...
      selectedItem.salary,
      // selectedItem.plannedDate, // Store Planned Date in column M of LEAVING sheet
    ];

    const leavingResponse = await fetch('https://script.google.com/macros/s/AKfycbyWlc2CfrDgr1JGsJHl1N4nRf-GAR-m6yqPPuP8Oggcafv3jo4thFrhfAX2vnfSzLQLlg/exec', {
      method: 'POST',
    ...
    if (!leavingResult.success) {
      throw new Error('Failed to insert into LEAVING sheet: ' + (leavingResult.error || 'Unknown error'));
    }

    // Second: Update JOINING sheet using updateCell action for specific columns
    // We need to find the row index first by fetching JOINING data
    const joiningResponse = await fetch('https://script.google.com/macros/s/AKfycbyWlc2CfrDgr1JGsJHl1N4nRf-GAR-m6yqPPuP8Oggcafv3jo4thFrhfAX2vnfSzLQLlg/exec?sheet=JOINING&action=fetch');
    const joiningResult = await joiningResponse.json();
    
    if (!joiningResult.success) {
      throw new Error('Failed to fetch JOINING data: ' + (joiningResult.error || 'Unknown error'));
    }

    const joiningData = joiningResult.data;
    const headers = joiningData[5]; // Headers are at row 6 (index 5)
    
    // Find the employee row index by matching Employee ID in column B (index 1)
    let rowIndex = -1;
    for (let i = 6; i < joiningData.length; i++) {
      if (joiningData[i][1] === selectedItem.employeeNo) { // Column B is index 1
        rowIndex = i + 1; // +1 because sheet rows start at 1, not 0
        break;
      }
    }

    if (rowIndex === -1) {
      throw new Error('Employee not found in JOINING sheet');
    }

    // Use fixed column indexes for Leaving Date (AO = column 41) and Reason (AP = column 42)
    const leavingDateColIndex = 41; // Column AO
    const reasonLeavingColIndex = 42; // Column AP

    const updateDateResponse = await fetch('https://script.google.com/macros/s/AKfycbyWlc2CfrDgr1JGsJHl1N4nRf-GAR-m6yqPPuP8Oggcafv3jo4thFrhfAX2vnfSzLQLlg/exec', {
      method: 'POST',
      body: new URLSearchParams({
        sheetName: 'JOINING',
        action: 'updateCell',
        rowIndex: rowIndex.toString(),
        columnIndex: leavingDateColIndex.toString(),
        value: formatDOB(formData.dateOfLeaving)
      }),
    });

    const updateDateResult = await updateDateResponse.json();
    if (!updateDateResult.success) {
      throw new Error('Failed to update Leaving Date: ' + (updateDateResult.error || 'Unknown error'));
    }

    const updateReasonResponse = await fetch('https://script.google.com/macros/s/AKfycbyWlc2CfrDgr1JGsJHl1N4nRf-GAR-m6yqPPuP8Oggcafv3jo4thFrhfAX2vnfSzLQLlg/exec', {
      method: 'POST',
      body: new URLSearchParams({
        sheetName: 'JOINING',
        action: 'updateCell',
        rowIndex: rowIndex.toString(),
        columnIndex: reasonLeavingColIndex.toString(),
        value: formData.reasonOfLeaving
      }),
    });

    const updateReasonResult = await updateReasonResponse.json();
    if (!updateReasonResult.success) {
      throw new Error('Failed to update Reason of Leaving: ' + (updateReasonResult.error || 'Unknown error'));
    }
