...
const Sidebar = ({ isOpen = false, onClose }) => {
  ...
  const adminMenuItems = useMemo(() => [
    ...
    { path: '/commercial-head-approval', icon: LeaveIcon, label: 'Hod Approval' },
  ...
  return (
    <>
      ...
      <div className={`hidden md:block lg:hidden fixed inset-0 z-[60] transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div
      ...
      <div className={`md:hidden fixed inset-0 z-[60] transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div
          className="fixed inset-0 bg-black bg-opacity-50"
          onClick={closeSidebar}
        />
